...
do
  ...
  ${HOME}/.conda/envs/dti/bin/python ${fCNNDir}/scripts/combine_tfMRI_volumes.py ${SubjectsConfig} ${Dir}
  # sample the nifti volumes to the surface
  ${HOME}/.conda/envs/dti/bin/python ${fCNNDir}/scripts/sample_volume_to_surface.py --hcp_dir ${Dir} --config ${SubjectsConfig} --method ${Method} --volume_template "T1w/Results/tfMRI_ALL/tfMRI_ALL_hp200_s2_level2.feat/{subject}_tfMRI_ALL_level2_zstat_hp200_s2_${Basename}.nii.gz" --task_names $fCNNDir/data/labels/ALL-TAVOR_name-file.txt --surface_name $SurfaceName
done
