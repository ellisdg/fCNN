...
def make_average_cifti_volume(config, directory, output_directory, subset, output_basename, reference_volume=None,
    ...
    for target_basename in target_basenames:
        average_volume_filename = make_average_cifti_volume_for_target(target_basename, output_directory,
                                                                       output_basename, subset, subject_ids, directory,
                                                                       reference_volume, overwrite=overwrite)
...
def make_average_cifti_volume_for_target(target_basename, output_directory, output_basename, subset, subject_ids,
    ...
    output_filename = os.path.join(output_directory,
                                   output_basename + os.path.basename(
                                       target_basename.format(subset)).replace(".volume.dscalar", ""))
    ...
    if overwrite or not os.path.exists(output_filename):
        output_filename = compute_average_image(image_filenames, output_filename)
    return output_filename
